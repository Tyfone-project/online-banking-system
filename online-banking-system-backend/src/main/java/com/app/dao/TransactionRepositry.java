...
public interface TransactionRepositry extends JpaRepository<Transaction, Integer> {
	...
	@Query(value = "select transaction_id AS transactionId, transaction_from as transactionFrom, transaction_to as transactionTo, amount, date, transaction_Status as transactionStatus FROM transactions where transaction_from=:accountNo ORDER BY date DESC LIMIT 5", nativeQuery = true)
